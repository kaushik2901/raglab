...
func (w *IndexWorker) Work(ctx context.Context, job *river.Job[IndexArgs]) error {
	if err := w.Store.runStep(ctx, job.Args.WorkflowID, "index", func(ctx context.Context, state map[string]any) (*types.StageResult, error) {
		inputDir := path.Join("artifacts", "preprocessing", job.Args.InputTag, "output")
		...
		embedder := embedder.New(llmBaseURL, llmAPIKey, job.Args.EmbeddingModel, job.Args.BatchSize)
		chunker := chunker.NewFixedChunker(job.Args.ChunkSize, job.Args.ChunkOverlap)
